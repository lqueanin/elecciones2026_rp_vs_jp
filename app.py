import os
import sqlite3
from flask import Flask, jsonify, render_template, request
from datetime import datetime

app = Flask(__name__)

# Configuración del volumen persistente en Railway
if os.path.exists('/data'):
    DB_PATH = '/data/elecciones.db'
else:
    DB_PATH = 'elecciones.db'

def get_db_connection():
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    return conn

def init_db():
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('''
        CREATE TABLE IF NOT EXISTS historial_brecha (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            fecha TEXT,
            votos_rp INTEGER,
            votos_jpp INTEGER,
            diferencia INTEGER
        )
    ''')
    conn.commit()
    conn.close()

@app.route('/api/push-votos', methods=['POST'])
def recibir_votos():
    # Token para evitar que extraños manipulen tu DB
    if request.args.get('token') != "unjbg_esis_2026":
        return jsonify({"status": "error"}), 401
    
    data = request.json
    try:
        v_rp = int(data['votos_rp'])
        v_jpp = int(data['votos_jpp'])
        dif = abs(v_rp - v_jpp)
        hora_actual = datetime.now().strftime('%H:%M')

        conn = get_db_connection()
        c = conn.cursor()
        # Evitamos duplicar datos si la hora es la misma
        c.execute('SELECT id FROM historial_brecha WHERE fecha = ? AND diferencia = ?', (hora_actual, dif))
        if not c.fetchone():
            c.execute('INSERT INTO historial_brecha (fecha, votos_rp, votos_jpp, diferencia) VALUES (?, ?, ?, ?)', 
                      (hora_actual, v_rp, v_jpp, dif))
            conn.commit()
        conn.close()
        return jsonify({"status": "success"})
    except:
        return jsonify({"status": "error"}), 400

@app.route('/api/historial')
def obtener_historial():
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('SELECT fecha, votos_rp, votos_jpp, diferencia FROM historial_brecha ORDER BY id ASC')
    filas = c.fetchall()
    conn.close()
    return jsonify([dict(f) for f in filas])

@app.route('/')
def index():
    return render_template('index.html')

init_db()

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port)
